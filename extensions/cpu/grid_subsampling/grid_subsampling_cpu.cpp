...
//    floor((maxCorner.y - originCorner.y) * sub_scale) + 1
  ...
  std::unordered_map<std::size_t, SampledInst> data_inst;
...
//    iZ = static_cast<std::size_t>(floor((p.z - originCorner.z) * sub_scale));
    ...
    if (!data.count(mapIdx)) {
      ...
      data_inst.emplace(mapIdx, SampledInst());

    }

    data[mapIdx].update(p);
    data_inst[mapIdx].update(inst[i]);
    i = i + 1;
  }
  ...
  s_inst.reserve(data_inst.size());
  for (auto& v : data_inst) {

    std::map<int, int> histo_map;
    for (int i : v.second.inst_ids) {
        histo_map[i]++;
    }
    auto maxElem = std::max_element(histo_map.begin(), histo_map.end(),
        [](const std::pair<int, int>& a, const std::pair<int, int>& b) {
            return a.second < b.second;
        });
    s_inst.push_back(maxElem->first);
  }
...
void grid_subsampling_cpu(
  ...
  for (std::size_t b = 0; b < batch_size; b++) {
    ...
    std::vector<int> cur_insts = std::vector<int>(
      insts.begin() + start_index,
      insts.begin() + start_index + lengths[b]
    );
    std::vector<int> cur_s_insts;

    single_grid_subsampling_cpu(cur_points, cur_s_points, cur_insts, cur_s_insts, voxel_size);

    s_points.insert(s_points.end(), cur_s_points.begin(), cur_s_points.end());
    s_insts.insert(s_insts.end(), cur_s_insts.begin(), cur_s_insts.end());
