...
class SuperPointRescoreMatching(nn.Module):
    def __init__(self, num_correspondences, dual_normalization=True):
        super(SuperPointRescoreMatching, self).__init__()
        self.num_correspondences = num_correspondences
        self.dual_normalization = dual_normalization

    def forward(self, ref_feats, src_feats, ref_masks=None, src_masks=None, ref_insts=None, src_insts=None, sg_matches=None, k=None):
        r"""Extract superpoint correspondences.
        ...
        # Super-point Rescoring Method
        ref_insts = ref_insts[ref_indices]
        src_insts = src_insts[src_indices]
        grid_ref, grid_src = torch.meshgrid(ref_insts.squeeze(), src_insts.squeeze(), indexing='ij')
        c = sg_matches[:, grid_ref.reshape(-1).long(), grid_src.reshape(-1).long()].view(matching_scores.shape[0], matching_scores.shape[1])
        matching_scores = matching_scores + 0.2*c #0.15*c/torch.sqrt(k)
